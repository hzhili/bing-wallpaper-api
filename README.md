...
#### "/" 接口
    ...
    - 出参: 重定向至URL
##### 接口说明: 
     接口从data目录下读取 location-today.json 文件,从文件中取出url, 并重定向至URL.

#### "/random" 接口
    - 入参: location  可选参数 请查看settings.py 中的 LOCATION 字典
    - 出参: 重定向至URL
##### 接口说明: 
     接口从data目录下读取 location.json 文件, 从文件中取出所有数据,从读取的数据中随机取一个url , 并重定向至URL.
     如果没有location参数,则随机选择一个location. 然后从文件中取出所有数据,从读取的数据中随机取一个url , 并重定向至URL.
